    ...
    public class ReviewsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ReviewDTO>>> GetReviews([FromQuery] QueryParameter queryParameter)
        {
            ...
            try
            {
                ...
                .Where(rev => rev.UserID == int.Parse(userId))
                .OrderBy(rev => rev.DateTime)
        ...
        public async Task<ActionResult<ReviewDTO>> PostReview(ReviewDTO reviewDTO)
        {
            ...
            bool reviewExists = await _context.Reviews.AnyAsync(rev => rev.UserID == int.Parse(userId) && rev.BookID == reviewDTO.BookID && (rev.Text == reviewDTO.Text || rev.Status == reviewDTO.Status));
            if (reviewExists)
            {
